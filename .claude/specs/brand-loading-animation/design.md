...
### LoadingSplash (`apps/web/src/components/LoadingSplash.tsx`) — 2026-07-20 추가
- **책임**: `fixed inset-0 z-40` + `bg-app` 불투명 배경 + 뷰포트 정중앙 `<BrandLoader />`. 부팅·캔버스 공용.
  - flow 안 배치(min-h 어림)는 헤더·사이드바만큼 치우치는 문제가 있어 fixed로 정중앙 보장.
  - div가 포인터 이벤트를 삼키므로 아래 UI 조작이 자연 차단됨 (R3.3)

### 캔버스 로딩 (`apps/web/src/features/canvas/index.tsx` 수정)
- **책임**: `useCanvasStore((s) => s.isLoading)` 구독, true면 `<LoadingSplash />` 렌더 (R3.1 — 풀스크린으로 변경)
  - `isLoading` false → 렌더 자체가 사라짐, 잔여 DOM 없음 (R3.2)
- **근거 요구사항**: R3.1–R3.3

### BrandMark (`apps/web/src/components/BrandMark.tsx`) — 2026-07-20 추가
- **책임**: BrandLoader와 동일 지오메트리의 정적 M 마크(애니메이션 없음). GlobalHeader 로고에 사용 (R4.1). 파비콘(R4.2)은 feat/web-favicon 브랜치 자산을 동일 도형으로 갱신.

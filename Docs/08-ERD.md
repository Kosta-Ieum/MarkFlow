...
> **저장 전략 메모** — 기획서 초안의 "캔버스 JSONB 통째 저장"은 PRD의 정규화 결정으로 대체한다. 정규화 시 노드 단위 락·히스토리·복구가 자연스럽고, Liveblocks Storage ↔ DB 영속화도 노드 row 단위로 매핑된다.
...
- Liveblocks Storage(LiveMap)는 위 프론트 객체를 그대로 보관하고, 서버 debounce(약 2초) 시 DB row로 영속화한다.
